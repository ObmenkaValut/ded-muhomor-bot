...
Если "ПОСЛЕДНЕЕ СООБЩЕНИЕ" — это просто короткое подтверждение или междометие («понятно», «понял», «ок», «ясно», «спс», «ага»), и ТЫ НЕ ЗАДАВАЛ ВОПРОС в предыдущем сообщении, то отвечать не нужно, разговор окончен (возвращай "reply": false).
